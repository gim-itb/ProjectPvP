...
public class PlayerDetection : MonoBehaviour
{
    ...
    private void PathFollow()
    {
        ...
        Gravity();
        isGrounded = Physics2D.Raycast(transform.position, Vector2.down, GetComponent<Collider2D>().bounds.extents.y + JumpOffset); 
        Vector2 direction = ((Vector2)path.vectorPath[currentWaypoint] - rb.position).normalized;
        Vector2 movement = EnemySpeed * Time.deltaTime* direction;
        if (isGrounded && (direction.y >= JumpHeightLimit))
        {
